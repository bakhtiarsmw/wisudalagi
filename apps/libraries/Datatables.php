...
class Datatables
{
    ...
    private function get_ordering()
    {
        ...
        if ($this->ci->input->post('order')[0]['column'] > 0)
    ...
    /**
     * public function to create array for to be
     *
     * @param type $param
     */
    public function formatArraySearch($param)
    {
        $filter = array();//print_r($param);die();

        for($i = 0; $i < count($param['columns']); $i++)
        {
            foreach($param['columns'][$i] as $key => $value)
            {
                if($param['columns'][$i]['searchable'] == 'true')
                    if(strlen($param['columns'][$i]['name']) > 0)
                        $filter['alphanumeric'][$param['columns'][$i]['name'] . ' ::text iLIKE'] = '%'. $param['search']['value'] .'%';
            }
